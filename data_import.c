...
int main(int argc, char *argv[]) {
    ...
    sqlite3_exec(g_db,
        ...
        "FROM ("
        "  SELECT q.date AS date, json_object("
        "    'symbol', q.symbol,"
        "    'name', coalesce(s.name, q.symbol),"
        "    'market', s.market,"
        "    'open', round(q.open, 4),"
        "    'high', round(q.high, 4),"
        "    'low', round(q.low, 4),"
        "    'close', round(q.close, 4),"
        "    'change', round(CASE WHEN q.prev_close IS NOT NULL AND q.prev_close != 0 THEN q.close - q.prev_close ELSE 0 END, 4),"
        "    'pct', round(CASE WHEN q.prev_close IS NOT NULL AND q.prev_close != 0 THEN (q.close - q.prev_close) / q.prev_close * 100 ELSE 0 END, 4),"
        "    'volumeRatio', round(CASE WHEN q.avg_volume_5 IS NOT NULL AND q.avg_volume_5 > 0 THEN q.volume / q.avg_volume_5 ELSE NULL END, 4),"
        "    'volume', q.volume,"
