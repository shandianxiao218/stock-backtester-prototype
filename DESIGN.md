...
`quote_snapshots` 目前预计算 `2025-01-01` 之后的交易日，覆盖前端日期选择范围。服务端命中后直接返回 JSON 字节，避免 Python 逐行组装 5000 条行情。
...
| 全市场行情 | 秒级 | 84.68ms 平均 | `quote_snapshots` 直接返回 |
