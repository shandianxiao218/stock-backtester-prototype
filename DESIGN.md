...
- [ ] 十字光标主图显示日期、开盘、收盘、最高、最低、涨跌幅、成交量，并放到顶部读数区，避免遮挡K线。
- [ ] 十字光标在成交量和各副图显示对应指标值。
- [ ] 指标名称后显示参数，例如 `MACD(12,26,9)`、`RSI(6,12)`。
- [ ] 双击指标线/指标图区域弹窗修改指标参数。
- [ ] 股票日线图标记买卖点。

### 历史周期与数据范围

- [ ] 前端 K 线请求起始日期从 `2025-01-01` 改为更早日期，允许查看 2025 年以前数据。
- [ ] 支持日线、周线、月线切换。
- [ ] 周线/月线由日线按周期聚合，遵守后端 `as_of_date` 截断后的数据范围。

### 股票列表与键盘导航

- [ ] 小键盘/键盘输入股票代码或拼音首字母，快速跳转个股K线。
- [ ] 左侧增加“全部 / 自选 / 持仓”列表。
- [ ] 自选股支持添加/移除并保存在本地。
- [ ] 个股 K 线页使用 PageDown / PageUp 按左侧当前列表切换下一只/上一只股票。
